...
운영을 시작한 뒤에는 두 가지를 더 알게 됐다. 첫째, webhook이 없는 구성에서는 GitHub에서 직접 만든 커밋이 CMS 목록에 나타나기까지 캐시 확인 주기만큼 기다려야 한다. 이것을 배포 실패로 착각하기 쉬웠다. 둘째, 블로그의 일반 Astro 빌드에 이력서 PDF 생성을 묶어두면 글 하나를 쓸 때도 Python과 RenderCV가 필요해진다. 이력서 YAML이 바뀔 때만 PDF를 생성하는 workflow로 분리하니 일반 배포가 다시 단순해졌다.
