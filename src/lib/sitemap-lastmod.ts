...
function getLastModifiedDate(frontmatter: BlogFrontmatter, filePath: string) {
  ...
  return getPostDateTime({
    ...
    timezone: updatedDate
      ? (optionalString(frontmatter.updatedTimezone) ??
...
function getBlogPath(
  ...
  const translationOf = optionalString(frontmatter.translationOf);
  if (translationOf) return `/blog/${translationOf}/en`;

  const relativePath = relative(contentDirectory, filePath)
    .split(sep)
    .join("/");
